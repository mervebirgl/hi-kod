...
veri = pd.read_csv(r"C:\Users\merve\OneDrive\Masaüstü\olimpiyatlar.csv")
...
plt.scatter(erkek.boy,erkek.kilo,alpha=0.5,color="orange",label="erkek")
plt.xlabel("boy")
plt.ylabel("kilo")
plt.title("Boy v kilo arasındaki fark")
plt.legend()  

numeric_correlation= veri.loc[:,["yas","boy","kilo"]].corr()

        
 # madalya ve yas arasındaki correlationa bakalım       
...
# üst sınır bu sinirın üstünde kalan degerler outlier
    ust_sinir=Q3 + outlier_step
# alt sınır bu sinirın altında  kalan degerler outlier
    alt_sinir=Q1 - outlier_step

    outlier_list_col= yas[(yas<alt_sinir)| (yas> ust_sinir)].index  
